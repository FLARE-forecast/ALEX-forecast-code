...
generate_salt_inflow_fc <- function(config,
  ...
  met_combined_first <- met_combined |>
    tidyr::pivot_wider(names_from = variable, values_from = prediction) |> 
    dplyr::mutate(date = lubridate::as_date(datetime)) #|>
  
  print(met_combined_first, n = 100)
  ...
  salt_training_df <- training_df |> 
    dplyr::filter(variable == "SALT", inflow_name == 'murray') |> 
    dplyr::select(-precip) |> 
    # add in a lagged predictor of lag(obs)
    dplyr::mutate(lag_obs = lag(observation, 1)) |> 
    dplyr::select(any_of(c('date', 'precip', 'temperature', 'threeday_precip', 'threeday_temp', 'doy', 'observation', 'lag_obs')))
  
  salt_rec <- recipe(observation ~ lag_obs + doy + threeday_temp + threeday_precip,
  ...
  folds <- vfold_cv(salt_training_df, v = 5) # orginally set to 10
  ...
  xgboost_inflow_fit <- fit(final_wrorkflow, data = salt_training_df)
  
  
  
  #make empty dataframe to store predictions
  salt_forecast <- tail(salt_training_df, 1) |> 
  ...
  for (i in 1:length(forecast_dates)) {
    ...
    previous_prediction <- salt_forecast |> dplyr::filter(date == forecast_dates[i] - days(1)) |> pull(prediction)
    ...
    salt_forecast <- dplyr::bind_rows(salt_forecast, ens_predictions)
  ...
  current_ens <- salt_forecast |> dplyr::distinct(parameter) |> pull() |> length() # how many are there?
